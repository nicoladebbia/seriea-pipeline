...
def test_config_endpoint_persists_and_reload_reads_it_back(monkeypatch):
    ...
    assert lm.load_live_poll_interval() == 60


def test_no_saved_choice_falls_back_to_the_given_default(monkeypatch):
    import scripts.pipeline.pipeline_state as ps
    monkeypatch.setattr(ps, "load_state", lambda: {})
    assert lm.load_live_poll_interval() == lm.AUTO_POLL_DEFAULT_S
    assert lm.load_live_poll_interval(default=120) == 120
...
def test_saved_choice_is_clamped_and_garbage_ignored(monkeypatch):
    ...
    assert lm.load_live_poll_interval() == 30
...
def test_live_window_opens_30_min_before_kickoff_and_closes_3h_after():
    ...
    gate now decides, and the live loop process reads it every minute."""
    ...
    assert lm.live_window_open(now, ko(4), 0.0) is True         # T-4: arm
    assert lm.live_window_open(now, ko(6), 0.0) is False        # T-6: not yet — the scores feed is empty before the whistle
    assert lm.live_window_open(now, ko(-149), 0.0) is True      # long stoppage: still armed
    assert lm.live_window_open(now, ko(-151), 0.0) is False     # long over
    assert lm.live_window_open(now, [], 0.0) is False
    assert lm.live_window_open(now, [{"match": "broken"}], 0.0) is False   # bad row -> closed, never a crash
    ...
    assert lm.live_window_open(now, k, stopped_after_100) is False
    ...
    assert lm.live_window_open(now, k, stopped_pre_kickoff) is True
    # ...and a later kickoff is judged on its own clock
    later = ko(-120) + ko(2)
    assert lm.live_window_open(now, later, stopped_after_100) is True
