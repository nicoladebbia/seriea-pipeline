...
| `data/monitoring/sofascore_cooldown_www.json` | same client, `www.sofascore.com` tier (HTML pages and the `/api/v1` proxy) | `sofascore_standings.sofascore_get_retry` (also the match-HTML scraper in `web/app.py`) | same shape; the two tiers are denied independently (the API-only "challenge" shape leaves www answering). |
