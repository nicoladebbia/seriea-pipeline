...
| `simulation.json` | 10k-run Monte Carlo: per-team group win/runner-up/best-third/advance, reach-R32→final, champion probabilities; most-likely R32 ties; **`bracket`** = the single most-likely tournament feeding the /worldcup bracket view — greedy group standings + Annex C thirds, then per-KO-match engine predictions (90' 1X2, top scorelines, advance incl. ET/pens), each match carrying `pairing_prob`, top-3 alt ties and win-slot marginals (third place included; `resolved: true` + pairing_prob 1.0 once knockout.py fills real names) | same generator (`--sims N`) | with predictions.json |
