...
- **The actual current squad per Serie A club** (distinct from the transfer-FLOW view): `team, player_name, position, age, market_value_eur, nationality`. 663 players / 20 clubs (2026-07-14). This is TM's live squad page — last season's stayers + this window's arrivals − departures, already resolved. Verified current (Højlund in Napoli, Simeone out).
- **Writer:** `scraper/transfermarkt.scrape_squad_market_values(season, league, only_teams)` → `_parse_squad_page`. Squad lists include primavera/youth on the senior page (counts run 25–43).
