...
- **Fallback for the fallback (2026-09-05):** when Sofascore's incidents/statistics endpoints are challenged, `matchday_updater --heal-espn` fills incidents (`source=espn`) and the team-stat columns of `matches.parquet` (`data_source=espn`; possession, shots, SoT, corners, fouls, saves, offsides, tackles, interceptions, clearances, passes, crosses, long balls, HT score, card counts — no xG) from ESPN's post-match summary. `check_match_record_completeness` warns when a finished match is still incomplete >14h after kickoff.
...
- **weather — timeouts write POISONED cache rows (fixed 2026-09-06):** an Open-Meteo timeout used to write a row with every `weather_*` NaN, which then counted as cached forever; 1,998 such rows (EPL 1,047 — all of 2024-25 — SA 951, mostly pre-2017) were dropped and the 2017+ ones re-fetched. A NaN weather cell today means the fetch has not run, not that it failed. Detection: rows where all `weather_*` are NaN.
- **Data-gap audit 2026-09-06 (Sofascore challenged, football-data 503) — what was filled from disk:** `matches.parquet` possession / xG / offsides / saves / interceptions / clearances / throw-ins / goal-kicks were NaN for every EPL match and every SA match before 2025-26 while `match_team_stats*.parquet` held them (agreement 1.000 on the overlap) — filled NaN-only for ~6,300 matches; EPL `shots_total` too (99.6% agreement), SA `shots_total` NOT (football-data and Sofascore count shots differently, 24% disagree). Half-time score for 253 rows from the fixture calendar's `period1`; `*_ht_goals`/`ht_result` is the pair features read, `*_ht_score` is the Sofascore/ESPN writers' pair — the heal now writes both. Verona 2024-25 was keyed `Hellas-Verona` in weather / FBref player_stats / goalkeeper_stats / lineups (38 matches) — re-keyed. `all_shots_with_xg_premier_league.parquet` built from the JSONs (85,724 shots, 2017-27) and every shot-level reader now loads both leagues (`features._utils.load_shot_level_xg`). EPL 2026-27 squad values fetched (Coventry added to `EPL_TEAMS_TM`). Still open: 85 historical matches with no Sofascore JSON (`failed_matches*.json`, `no_lineup_data`), 2018-19 shotmaps (SA 39 / EPL 29), football-data CSVs for the 2025-26 tail (SA after 27 Apr, EPL after 13 Apr) and 2026-27, Understat `matches_xg` 2026-27 (site lists no played matches). Findings: `.plans/data-gaps-2026-09-06-findings.md`.
- **Multi-source finished-match record (2026-09-06, `scripts/data/match_record_chain.py`):** the four per-match stat parquets (`player_match_stats*`, `match_team_stats*`, `all_shots_with_xg*`, `shotmap_stats*`) now carry a `source` column — `sofascore` (or NaN on rows older than the column) is the record; `fotmob` / `espn` rows are STAND-INS written by the chain when the Sofascore ingest left a finished match empty (403 challenge / IP deny). A stand-in is not coverage: the matchday detector re-fetches the match from Sofascore when it answers and `_save_merged(replace_stand_ins=True)` / `write_shot_level_xg.rebuild_from_cache` drop the stand-in rows then. Stand-in player ids are foreign: resolved to the Sofascore id when the parquet's own history has exactly one player of that folded name in that team, otherwise `-fotmob_id` (negative). FotMob carries every player stat family except carries / progression / possession-lost / Sofascore's value models (those stay NaN, never 0) and a full shot map (xG, xGOT, situation, body part; coordinates converted from metres to Sofascore's percent-from-goal-line); ESPN carries shots / SoT / goals / assists / fouls / cards / saves + minutes and the ALL-period boxscore only. Raw payloads: `data/external/fotmob/match_details/matches{_premier_league}/{season}/{fotmob_id}.json` (the sibling `data/external/fotmob/matches/` is a legacy 2017-26 dump of `{"basic": ...}` stubs from a scraper no longer in the tree — different shape, not reused). Per-run report: `data/monitoring/ingest_chain_status.json` (per league: every match with a missing component, what filled it, why a source declined). Sofascore ingest cooldown: `data/monitoring/sofascore_cooldown.json` (60 min after the first denial of a run — retry storms earned the IP deny). First real run: EPL 9 matches × shot maps from FotMob (229 shots, 218 player ids resolved); Serie A needed nothing once the cached Sofascore shot maps were rebuilt into the parquet.
...
| `data/upcoming/pick_markets_raw.json` | `odds_fetcher.fetch_pick_markets` (T-6h/T-3h stages + pre-kickoff, 45-min gate/event) | `{fetched_at, league, events{<odds_api_event_id>: {home, away, home_raw, away_raw, commence, fetched_at, bookmakers[{title, markets[{key, outcomes[{name, description, point, price}]}]}]}}}` | RAW outcome naming kept verbatim (team names in `h2h_h1`, `Juventus/Draw` in HT/FT, `Juventus:1\|AC Milan:0` in correct score, full player names in `description`). 10 markets, 10 credits/event (was 16; corners/cards alt totals, first/last scorer and card props dropped 2026-09-05, nothing read them; `btts_h1` + `double_chance_h1` now priced by the simulator rows `Goal 1° tempo` / `Doppia chance 1° tempo`). |
| `data/betting/picks_journal.json` | `picks.journal_lean` (T-30 only, kickoff ≤ 3h; headline + best exotic + every in-band angle, dedup per market+date) | bet_journal schema + `extra{bet_type, player, team, source, tier, side, line, lineup, start_pct}`; `market` = Odds API key, `selection` embeds the player; `sharp_implied_prob` is null (1/odds of the placed price made every CLV a fake 0.0 until 2026-09-05); `closing_odds` set by the grader from the feed's last price | Flat €10 paper. Settled by `picks.settle_picks` from auto_settle; `picks.picks_record()` = per-market ROI/CLV bar. Never touches bankroll/history. |
| `data/betting/market_promotion.json` | `market_promotion.evaluate_promotions` (after every `settle_picks`; idempotent) | `{updated_at, bar, demotion_bar, markets{<market key>: {status: paper\|promoted, since, record_from, paper{n, won, roi_pct, z, profit, mean_clv_pct, n_clv}, real{...}\|null, distance, reason, snapshot, promoted_at, demoted_at}}}` | Which paper markets have earned REAL stakes (≥50 settled, ROI>0, z≥1, CLV>0 when ≥20 closing prices). A promoted market's LEAN is mirrored into `bet_journal.json` (`pipeline_status: pick:promoted`, `extra.picks_ref`, Kelly×0.5 cap 1.5%) and settled with its paper entry. Rendered by `/record` and the Monday digest. **Never hand-edit** — the gate is the product. |


### Reliability state files (added 2026-09-06)

| File | Writer | Reader | What it is |
|---|---|---|---|
| `data/monitoring/live_loop_status.json` | `scripts/data/live_monitor.py --loop` (the `com.seriea-pipeline.live-loop` launchd job) on every state change and every 15s | `web/app.py` (`/api/live`, scheduler status), `health_check.check_live_loop` | `active`, `interval`, `next_at`, `fast_interval`, `fast_at`, `has_live`, `stopped_at`, `polls`, `heartbeat` (epoch), `pid`. A heartbeat older than 180s reads as a dead loop (`alive: false`, `active` forced false). |
| `data/monitoring/sofascore_cooldown.json` | `scraper/sofascore_client.py` on the first 401/403/429 from `api.sofascore.com` | every Sofascore caller through `sofascore_client.get_json`; `matchday_updater` (ingest skip) | `{tier, set_at, until_ts, minutes, reason}`; 60 min; never extended by a second denial. |
| `data/monitoring/sofascore_cooldown_www.json` | same client, `www.sofascore.com` tier (HTML pages and the `/api/v1` proxy) | `sofascore_standings.sofascore_get_retry` (also the match-HTML scraper in `web/app.py`), the World Cup fetcher | same shape; the two tiers are denied independently (the API-only "challenge" shape leaves www answering). |
| `data/betting/history.json` | `scripts/betting/ledger.rebuild_caches()` | nobody on the web since 2026-09-06 (`ledger.get_history_view()` derives the same view from `bet_journal.json`) | courtesy snapshot for humans; never a source of truth. |
| `pipeline_state.json` keys `live_loop_request`, `live_poll_interval`, `live_fast_interval` | `web/app.py` (`/api/live/auto-poll`, `/api/live/config`) | the live loop, on its next tick / next poll | `{"action": "start"|"stop", "at": iso}` consumed once by its timestamp; the intervals chosen on `/live`. The old `sofascore_api_blocked_until` key is dead (cooldown file replaces it). |
