...
def build_player_xg_features(matches: pd.DataFrame, player_db: PlayerXGDatabase = None) -> pd.DataFrame:
    """Add player-based xG features to match DataFrame.

    Features added:
    - home_lineup_xg, away_lineup_xg (predicted from players)
    - home_top3_xg_share, away_top3_xg_share (squad dependency)
    - home_star_form, away_star_form (top contributors recent form)
    """
    ...
    lineup_predictor = LineupXGPredictor(player_db)

    df = matches.copy()

    # Initialize columns
    df["home_lineup_xg"] = 0.0
    df["away_lineup_xg"] = 0.0
    df["home_star_xg_form"] = 0.0
    df["away_star_xg_form"] = 0.0
    df["home_squad_xg_depth"] = 0.0
    df["away_squad_xg_depth"] = 0.0
    ...
    for idx, row in df.iterrows():
        ...
        # Predict home team xG
        home_pred = lineup_predictor.predict_team_xg(home)
        df.at[idx, "home_lineup_xg"] = home_pred["predicted_xg"]

        # Predict away team xG
        away_pred = lineup_predictor.predict_team_xg(away)
        df.at[idx, "away_lineup_xg"] = away_pred["predicted_xg"]

        # Star player form (top 3 contributors)
        home_top3 = player_db.get_team_top_contributors(home, n=3)
        if home_top3:
            df.at[idx, "home_star_xg_form"] = np.mean([
                player_db.get_profile(p, home).recent_form_xg
                for p, _ in home_top3
                if player_db.get_profile(p, home)
            ])

        away_top3 = player_db.get_team_top_contributors(away, n=3)
        if away_top3:
            df.at[idx, "away_star_xg_form"] = np.mean([
                player_db.get_profile(p, away).recent_form_xg
                for p, _ in away_top3
                if player_db.get_profile(p, away)
            ])

        # Squad depth (xG capacity beyond top 3)
        home_total = player_db.get_team_total_xg_capacity(home)
        home_top3_xg = sum(s for _, s in home_top3) if home_top3 else 0
        df.at[idx, "home_squad_xg_depth"] = home_total - home_top3_xg

        away_total = player_db.get_team_total_xg_capacity(away)
        away_top3_xg = sum(s for _, s in away_top3) if away_top3 else 0
        df.at[idx, "away_squad_xg_depth"] = away_total - away_top3_xg

    log.info(f"Added player xG features to {len(df)} matches")
    return df
