...
- **Promotion gate (2026-09-05, `scripts/betting/market_promotion.py`) — a paper market earns
  ...
  **Closed the same day — the stake follows the record, not the constant.** An incumbent
  ...
  that do not trip `DEMOTION_BAR` **and whose ROI is positive**
  (`INCUMBENT_FULL_STAKE_MIN_ROI_PCT`, the same `<=` comparison `passes_bar` uses, so
  break-even is not positive); then full 0.15 / 2.5%. The ROI leg is Nicola's call of
  2026-09-08 and it changed the ladder: the only test at n=30 used to be `should_demote`
  (ROI < −10% or z < −1), so a record at ROI −9.9% DOUBLED the stake on the next slip —
  "not demonstrably bad" for the incumbents against "demonstrably good" (ROI > 0, z ≥ 2.5)
  for every prop queueing behind them. z is deliberately not required at n=30: that is the
  promotion bar's job at n=50, and demanding it here would pin the incumbents to half stake
  forever. Both renderers state the real condition — never let `/betting` or the `/record`
