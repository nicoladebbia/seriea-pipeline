...
🟢 `scraper/sofascore_client.py` — grade A · keep (added 2026-09-06). The ONE Sofascore HTTP client: `get_json(url)` with per-tier (`api` / `www`) file-backed cooldowns, no retry on a denial, backoff on 5xx/transport, `last_failure_status()`. Imported by scraper/sofascore_events.py (thin `_get_json` wrapper that mirrors `_LAST_FAILURE_STATUS`), scripts/data/live_sofascore.py, scripts/data/matchday_updater.py, scraper/sofascore_standings.py, scripts/fantacalcio/xi_advisor.py. To change how a Sofascore denial is handled, the file is this one. Tests: tests/test_sofascore_client.py.
...
- **Does:** Automated model retraining triggered by matchweek completion: auto-detects quick vs full Optuna retrain mode, archives old models, compares performance, and promotes new models on validation. The ensemble decision gates ONLY the ensemble: the auxiliary models (`catboost_no_odds`, xG, draw detector, and the O/U classifiers behind the only enabled betting markets) each carry their own gate and retrain on every run via `_retrain_auxiliary_models`; `_refresh_after_retrain` then re-runs predictions once, after every step, if anything was promoted — predictions.json via the engine, then goal_predictions.json via `over_under_model.refresh_from_predictions` (the file the O/U scanner prices, otherwise written only by pipeline Step 19) (since 2026-09-05 — before that the O/U retrain and the refresh sat inside the ensemble's promote branch).
...
- ⚠️ **The matchweek-1 cliff (measured 2026-08-02) — this is the dominant fact about this module.** Accuracy is not uniform across the season: every matchweek from 3 onward runs **73–81%**, while **matchweek 1 runs ~60%** — caused by the `season.max()` filter above, since at MW1 the "current" table is still **last** season. **Quote the arm you mean.** Controlled on the two seasons with friendly coverage (n=80 MW1 fixtures, same fixtures both arms): naive floor **48.6%**, friendlies **off 49.6%**, friendlies **on 60.6%** — so the pre-season signal is worth **+11.0pp at MW1** and is already shipped. Without it the model sits *at* the naive floor there (and *below* it on the 6-season calibration split, −0.65pp), which is the number to cite when arguing the signal's value — **not** as a description of production. The live cliff is 60.6% vs ~79%.
- ⚠️ **The pre-season signal is HARMFUL after matchweek 1** (same controlled measurement): rounds 2+ score **79.49% off vs 79.28% on, −0.21pp**. By round 2 real league data exists and the pre-season prior is stale noise. `PRESEASON_FADE_MATCHES = 5` keeps it alive for five matches, i.e. through rounds where it costs accuracy — which is why every sweep prefers a shorter fade, and why fade and penalty interact violently (a −80 penalty over 5 matches measures −0.829pp; the same penalty over 2 measures +0.568pp). Consequences for anyone tuning this file: (a) the scoring constants (`RECENCY_DECAY`, the window, the `LAST_MATCH_*` tiers) compete for ~0.05–0.5pp in the weeks that already work, and most of that does not survive leave-one-season-out — **don't re-run those hunts**, see `.plans/xi-constant-tuning-findings.md`; (b) every effect that did validate lives at MW1 and **nowhere else** — shrinkage is worth **+1.755pp at MW1 and +0.007pp after** (rounds 2+ are flat to three decimals across strengths 2→128, because by round 3 each player has enough current-season appearances that the prior's weight vanishes); (c) the pre-season friendly signal is measurable on only **2 of 8** backtest seasons — coverage starts 2024-25 — so any sweep averaged over all seasons understates it ~4×.
...
- **Both leagues, one producer (fixed 2026-08-02):** `generate_lineup_predictions()` reads **both** `predictions.json` and `predictions_premier_league.json` (deduped by match key) and writes the whole `lineup_predictions.json`. It used to read the Serie A file only; the EPL was served by a **second, independent XI producer** in `generate_epl_supplementary.generate_epl_lineup_predictions` — most-used starters over the last ten rounds, no recency decay, no shrinkage, no manager inertia, no friendlies — so every measured improvement in this module reached one league out of two. That function now **delegates** here and returns the EPL subset; the old read-modify-write merge at step 8 is gone. Two consequences: EPL formations are now predicted per club instead of taken from the ensemble's `formation_analysis`, which was measured to be the **constant `4-2-3-1` on all 20 EPL club-slots** (Serie A's varies, so the field works — the EPL half was never populated); and EPL lineups now refresh on the **incremental** pipeline too, where previously only a full run rebuilt them. Verified live: 10/10 SA and 20/20 EPL clubs produce a full 11.
- ⚠️ **`SHRINK_PRIOR_STRENGTH` and `PRESEASON_ABSENT_PENALTY` are COUPLED, and the legal penalty is a WINDOW that narrows as strength rises (2026-08-02).** Two semantic invariants bound the penalty from opposite directions: a pre-season *unused substitute* must outrank a player absent entirely (violated when the penalty is too shallow), and an established league regular who missed the friendlies must still outrank a fringe friendly participant (violated when it is too deep). Both are closed-form — a player with no league history is untouched by shrinkage, and the absent score is exactly `base(strength) + penalty` — giving legal bands of −66…−5 at strength 2, **−43.5…−18.4 at 16** (shipped: −30), −36.2…−22.9 at 32, −31.1…−26.0 at 64. Push strength far enough and **no** penalty satisfies both; that, not any hit rate, is the ceiling on shrinkage. The full table is in the code beside both constants — **re-solve it before changing either**. `PRESEASON_FADE_MATCHES` does not move the window (identical bounds at fade 1/2/3/5/8). Neither invariant was findable by sweeping: every sweep varied one constant with the others at production, so both were caught by `tests/test_lineup_preseason.py`. Inside the window the penalty is **inert on real data** — −45 and −30 give byte-identical MW1 XIs at every club in both friendly seasons.
...
#### 🟢 `scripts/betting/inplay.py` — grade A · keep (added 2026-09-05)
- **What:** the in-play PAPER engine. Fair 1X2 (and totals, display only) for every live Odds API snapshot from `goal_process.simulate_from_state` (score + minute + red cards on the board — `red_mult` in profile.json is MEASURED by `goal_process --measure-red` — pre-match-market baseline via `market_profile`, closing-snapshot and archived-xG fallbacks), edge vs the de-vigged in-play average, one paper pick per selection per match after a score change (`EDGE_MIN` 5%, `FAIR_MIN` 10%, `MAX_MINUTE` 85, 1X2 only — the totals lines in a snapshot are pre-match lines carried along), flat-stake journal `data/betting/inplay_journal.json`, settle at the whistle with CLV vs the NEXT snapshot, `--backtest` over `data/live/*.json` → `data/models/inplay/backtest.json`. Money never moves here.
- **Talks to:** Imports: goal_process.py (`simulate_from_state`, `market_profile`, `market_probs`), bet_journal.py (`add_bet`, `settle_bet`), betting_unified.py (`PAPER_STAKE`), notify.py (ping, `inplay_pings` state key, default off); Imported by: live_monitor.py (`poll_once` → `on_snapshot` per live snapshot, `settle_for_matchday` before save), web/app.py (`api_live` exposes `inplay_pings` + the backtest verdict; `api_live_config` sets the mode).
- **To change the rule:** the constants at the top; re-run `python3 -m scripts.betting.inplay --backtest` and read the gate before touching pings.
