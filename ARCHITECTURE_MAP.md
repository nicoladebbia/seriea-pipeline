...
- ⚠️ Sofascore club names are not canonical (`Liverpool FC`, `Man City`, `Milan`, `Napoli`). `predict_all_lineups` resolves canonical → Sofascore with a 3-step map (exact → `normalize_team` → substring). `normalize_team("Liverpool")` alone does **not** resolve; the substring step is load-bearing, not a nicety.
