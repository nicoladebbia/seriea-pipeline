...
from config.settings import DATA_DIR, KELLY_FRACTION, LEAGUE_KELLY_FRACTIONS, MAX_STAKE_PCT, MODELS_DIR
...
# =============================================================================
# The per-league fractions live in config.settings.LEAGUE_KELLY_FRACTIONS (one
# definition); the per-league scaler below divides by cfg.kelly_fraction, so
# serie_a MUST equal KELLY_FRACTION — tests/test_staking_config.py pins it.
_LEAGUE_KELLY_DEFAULTS = dict(LEAGUE_KELLY_FRACTIONS)


def _load_league_kelly_fraction(league: str, default: float = KELLY_FRACTION) -> float:
...
class BettingConfig:
    ...
    max_stake_pct: float = MAX_STAKE_PCT  # Maximum single bet (pro standard: 2-3%)
    ...
    market_rules: Dict = field(default_factory=lambda: {
        ...
        "O/U_Over":   {"enabled": True,  "min_edge_pct": 5.0,  "max_edge_pct": 7.0,    # O/U 1.5: +10.1% ROI (74% WR, 46 bets), realized CLV +2.5%.
                       "allowed_lines": [1.5, 2.5], "kelly_fraction": KELLY_FRACTION,             # 2026-06-01: PER-LINE shrinkage. 1.5 model claims ~8% edge but
