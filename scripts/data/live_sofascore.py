...
# The Sofascore 403 breaker is the shared client cooldown (scraper.sofascore_client,
# api tier, file-backed so it survives a restart and is seen by every process).
...
def fetch_live_data_for_matches(match_keys: list[str]) -> dict[str, dict[str, Any]]:
    ...
    cooling = _client.cooldown_remaining("api")
    blocked = cooling > 0
    id_map: dict[str, int] = {}
    if not blocked:
        ...
        log.info("Sofascore: 403 cooldown for %ds more, using ESPN", int(cooling))
        ...
        if sofascore_id is not None:
            ...
            if data is not None and not _answered(data):
                ...
                if status == 403 and not blocked:
                    _client.set_cooldown("live monitor: 403 on every endpoint", tier="api")
                    blocked = True
                    log.warning("Sofascore API 403 on every endpoint — cooling down, "
                                "serving live stats/events from ESPN")
                data = None
        if data is None:
