...
def main() -> int:
    ...
    # The walkforward fold models live at:
    # data/models/walkforward/serie_a/1x2__5season_seed42/season_<YEAR>.cbm
    import pickle as pk
    model_dir = PROJECT_ROOT / "data" / "models" / "walkforward" / "serie_a" / "1x2__5season_seed42"
    if not model_dir.exists():
        log.error("Walkforward model dir not found: %s", model_dir)
        return 1

    # Map each season to the latest fold model that doesn't include it in training.
    # For SA: 2022-23 → uses season_2022-2023.cbm (trained on prior). Same for
    # 2023-24, 2024-25. For 2025-26 (no fold model exists yet), use season_2024-2025.cbm
    # (trained on 2005-2023) — conservative since it doesn't see 2024-25 either.
    season_model_map = {
        "2022-2023": "season_2022-2023.cbm",
        "2023-2024": "season_2023-2024.cbm",
        "2024-2025": "season_2024-2025.cbm",
        "2025-2026": "season_2024-2025.cbm",  # latest available
    }

    all_probs = []
    for season in target_seasons:
        model_filename = season_model_map.get(season)
        if model_filename is None:
            log.warning("No model mapping for season %s", season)
            continue
        model_path = model_dir / model_filename
        cal_path = model_dir / model_filename.replace(".cbm", "_calibrators.pkl")
        ...
        meta_path = model_dir / model_filename.replace(".cbm", "_metadata.json")
        if meta_path.exists():
